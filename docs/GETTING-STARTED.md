...
> **اول غربال کن.** کرالر از پیش‌نمایش وب (`t.me/s/<نام>`) می‌خواند و بسیاری از
> کانال‌های موزیک آن را بسته‌اند. قبل از افزودن، فهرست را غربال کن تا فقط
> قابل‌کرال‌ها بمانند:
>
> ```bash
...
> ```
>
> خروجی سه حالت دارد: قابل کرال، پیش‌نمایش بسته، و کانالی که اصلاً وجود ندارد.
...
**کانالی که پیش‌نمایش ندارد چه؟** سه راه دارد.

۱. **ربات را در کانال ادمین کن** (مسیر B). فقط پست‌های جدید می‌آیند، آرشیو قدیمی نه،
   چون Bot API تاریخچه نمی‌دهد. برای کانال‌هایی که خودت دسترسی داری بهترین راه است.

۲. **fallback با MTProto** — یک اکانت تلگرام لاگین می‌کنی و کانال را همان‌طور می‌خواند
   که یک آدم با اسکرول کردن می‌بیند، با آرشیو کامل. پیش‌فرض **خاموش** است، چون ریسکی
   دارد که مسیر وب ندارد: تلگرام می‌تواند اکانت را محدود یا بن کند.

   ```bash
   # یک شمارهٔ جدا — نه شمارهٔ resolver، نه شمارهٔ شخصی خودت
   docker compose -f infra/compose/solo.yml --env-file .env \
     run --rm edge python -m tmusic_indexer.login crawl1
   ```

   نام سشن مهم است: هر چیزی که با `crawl` شروع شود مال کرالر است، بقیه مال resolver
   (همان که پخش را ممکن می‌کند). این جداسازی عمدی است — اگر اکانت کرالر محدود شود،
   پخش کاربران نباید با آن بخوابد.

   بعد در پنل ادمین → **سیستم** → فلگ `mtproto_fallback` را روشن کن. از آن لحظه
   کانال‌هایی که `preview_disabled` خورده‌اند خودکار به این مسیر منتقل می‌شوند؛ لازم
   نیست دستی «کرال مجدد» بزنی.

۳. **رهایش کن.** صف کاندیداها خودش این تشخیص را می‌دهد: هر کاندیدی که probe شود و
   پیش‌نمایش نداشته باشد خودکار `rejected` می‌شود و در صف ادمین نمی‌آید.

وقتی یک کانال قابل‌کرال داخل شد، لازم نیست بقیه را دستی پیدا کنی: کرالر از متن
پست‌ها نام کانال‌های دیگر را برمی‌دارد، probe می‌کند و فقط قابل‌کرال‌ها را با امتیاز
در صف می‌گذارد.
...
دو چیز لازم دارد و هر دو یک‌بار برای همیشه است:

۱. `TG_API_ID` و `TG_API_HASH` در `.env` (از my.telegram.org → API development tools).
   اینها اعتبارنامهٔ **اکانت** هستند، نه توکن ربات؛ ربات نمی‌تواند فایلی را که هرگز
   دریافت نکرده دانلود کند. بعد از تغییر `.env`، کانتینر باید **بازساخته** شود
   (`restart` کافی نیست، چون env فقط موقع ساخت خوانده می‌شود):

   ```bash
   docker compose -f infra/compose/solo.yml --env-file .env up -d edge
   ```

۲. لاگین سشن resolver:

   ```bash
   docker compose -f infra/compose/solo.yml --env-file .env \
     run --rm edge python -m tmusic_indexer.login acc1
   ```
...
| «الان قابل پخش نیست» | `TG_API_ID`/`TG_API_HASH` ست نشده، یا اکانت resolver لاگین نشده، یا FloodWait خورده | [RUNBOOK → resolver مرده](RUNBOOK.md#resolver) |
