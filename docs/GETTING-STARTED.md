...
### روی یک سرور، برای کاربران واقعی

کامپوز توسعه همه‌چیز را فقط روی `127.0.0.1` باز می‌کند و TLS ندارد. برای اینکه
واقعاً قابل استفاده باشد، سه چیز لازم است:

۱. دو فرانت‌اند build شوند:
...
۲. یک reverse proxy با TLS جلویش باشد — تلگرام برای مینی‌اپ و وبهوک **حتماً**
HTTPS می‌خواهد. نگاشت: `app.<دامنه>` → `miniapp/dist`، `admin.<دامنه>` →
`admin/dist`، `api.<دامنه>` → `127.0.0.1:8000`، `cdn.<دامنه>` → `127.0.0.1:8081`.
نمونهٔ آمادهٔ nginx در `infra/nginx/core.conf.template` است.

۳. edge با آدرس عمومی دوباره ثبت شود:

```bash
CDN_URL=https://cdn.<دامنه> bash scripts/install.sh
```

اگر ترافیک زیاد شد یا خواستی استریم از خارج سرو شود، همان کد بدون تغییر به دو
سرور تقسیم می‌شود (بخش ۳).
