...
**فارسی‌بودن کاتالوگ یک تنظیم نیست، نتیجهٔ کانال‌هاست.** هیچ‌جای کد زبان را فیلتر
نمی‌کند: پارسر روی `Artist - Title` کار می‌کند نه روی الفبا، و `detect_language` فقط
زبان را *تشخیص* می‌دهد و ذخیره می‌کند (`fa`, `en`, `ar`, `tr`, `ku`) تا کاربر بتواند در
کتابخانه فیلتر کند. اگر کانال انگلیسی یا عربی اضافه کنی، موزیک انگلیسی و عربی هم
ایندکس می‌شود — همان مسیر، همان dedup.

یک نکته دربارهٔ عربی: حروف `ي`، `ك` و `ة` به شکل فارسی (`ی`، `ک`، `ه`) تبدیل می‌شوند.
این عمدی است و همان چیزی است که باعث می‌شود یک آهنگ با دو املای مختلف یک ترک شود، ولی
یعنی عنوان‌های عربی با حروف فارسی نمایش داده می‌شوند.
...
**راه چهارم — جست‌وجوی خود تلگرام** (نیاز به اکانت کرالر دارد). با فلگ
`telegram_search` روشن، edge هر پنج دقیقه یک کلمه از فهرست `music_search_terms` را
در تلگرام جست‌وجو می‌کند — هم پست‌های صوتی عمومی (`searchGlobal` با فیلتر موزیک) و هم
کانال‌هایی که اسمشان می‌خورد. نتیجه‌ها کانال کاندیدا می‌شوند، نه کانال تأییدشده: همان
صف، همان probe، همان تأیید ادمین.

کلمه‌ها را می‌شود عوض کرد بدون دیپلوی:

```sql
UPDATE settings SET value = '["نام خواننده", "ژانر", "remix"]' WHERE key = 'music_search_terms';
```

**راه پنجم — کاربرها**. هر فایل موزیکی که کسی برای ربات بفرستد ایندکس می‌شود، و چون
از Bot API می‌آید `bot_file_id` دارد — یعنی **بدون اکانت MTProto هم پخش می‌شود**. فوروارد
کردن یک پست از کانال هم همزمان کانال را کشف می‌کند و هم فایلش را ثبت می‌کند.
...
## ۳ـ. صفحهٔ خواننده و عکس‌ها

صفحهٔ هر خواننده سه بخش دارد: محبوب‌ترین‌ها (بر اساس لایک)، آلبوم‌ها، و همهٔ آهنگ‌ها.

**عکس خواننده از اسپاتیفای می‌آید** و اختیاری است. بدون کلید، همه‌چیز کار می‌کند
و فقط خواننده‌ها عکس ندارند.

۱. در [developer.spotify.com/dashboard](https://developer.spotify.com/dashboard) یک اپ بساز
   (هر اسمی) و `Client ID` و `Client Secret` را بردار.
۲. در `.env`:
...
۳. ورکر را بازبساز:
   ...
   ```bash
   docker compose -f infra/compose/solo.yml --env-file .env up -d --force-recreate worker
   ```

job `enrich_artists` هر بیست دقیقه ۲۰ خواننده را نگاه می‌کند، از پرترک‌ترین شروع
می‌کند، و هر خواننده را **فقط یک بار** می‌پرسد — چه پیدا شود چه نشود.

تطابق محافظه‌کارانه است: فقط وقتی عکس برداشته می‌شود که نام نرمال‌شده یا اسکلت
لاتین دقیقاً یکی باشد (پس «Moein» با «معین» می‌خورد ولی با «Moein Zandi» نه). عکس غلط
روی صفحهٔ خواننده بدتر از نبودن عکس است.

> تصاویر از CDN خود اسپاتیفای (`i.scdn.co`) لود می‌شوند و چیزی ذخیره نمی‌شود. اگر
> محصول را تجاری منتشر می‌کنی، شرایط توسعه‌دهندهٔ اسپاتیفای از جمله ذکر منبع را
> یک بار بخوان.

---

## ۴ـ. محدودیت‌ها و جوین اجباری

همه‌شان از پنل عوض می‌شوند (سیستم ← «محدودیت‌های پلن رایگان») و در دیتابیس ذخیره
می‌شوند، نه در کد. مقدار `-1` یعنی بی‌نهایت.

| محدودیت | رایگان | پرمیوم |
|---|---|---|
| پخش در روز | ۴۰ | بی‌نهایت |
| پلی‌لیست | ۲ | بی‌نهایت |
| کانال | ۱ | بی‌نهایت |
| ترک ذخیره‌شده | ۲۰۰ | بی‌نهایت |
| دانلود آفلاین | — | دارد |

**جوین اجباری:** فهرست کانال‌ها در پنل ← سیستم ← «کانال‌های جوین اجباری».
یک بار موقع باز شدن اپ چک می‌شود و جواب پنج دقیقه کش می‌شود؛ دکمهٔ «عضو شدم»
کش را پاک می‌کند و فوری دوباره می‌پرسد.

> **ربات باید در هر کانال ادمین باشد.** تلگرام `getChatMember` را فقط برای چتی جواب
> می‌دهد که خود ربات در آن باشد. اگر نباشد یا یوزرنیم غلط باشد، آن کانال نادیده
> گرفته می‌شود و در باز می‌ماند — تنظیم خراب نباید به قطعی محصول تبدیل شود.
...
| بعد از `git pull` همه‌چیز ۵۰۲ می‌دهد | قالب nginx عوض شده و کانتینرش از نو ساخته نشده | [RUNBOOK](RUNBOOK.md#nginx-update) |
