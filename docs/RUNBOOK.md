...
**همان شماره؟ می‌شود، ولی بدان چه می‌خری.** اگر موقع لاگین `crawl1` همان شمارهٔ
`acc1` را بدهی، تلگرام یک نشست دوم روی همان اکانت می‌سازد و کد بدون تغییر کار
می‌کند — دو سشن، یک اکانت. جداسازی فقط اسمی می‌شود: FloodWait و بن روی **اکانت**
اعمال می‌شوند نه روی سشن، پس اگر کرال باعث محدود شدنش شود، پخش کاربران هم با آن
می‌خوابد. با دو شمارهٔ جدا این اتفاق نمی‌افتد.
...
## <a id="tgsearch"></a>جست‌وجوی تلگرام برای کشف کانال

**چیست:** به‌جای انتظار برای اینکه کانالی به کانال دیگر لینک بدهد، اکانت کرالر از خود
تلگرام می‌پرسد. پشت فلگ `telegram_search` و پیش‌فرض خاموش.

**روشن کردن:** همان اکانت `crawl*` که برای fallback لازم است، بعد:

```sql
UPDATE feature_flags SET value = 'true' WHERE key = 'telegram_search';
```

**هزینه‌اش:** حداکثر یک جست‌وجو هر پنج دقیقه (`SEARCH_INTERVAL_S`). عمداً کند است —
رگبار جست‌وجو دقیقاً همان چیزی است که اکانت را محدود می‌کند. FloodWait مثل بقیهٔ مسیرها
اکانت را خنک می‌کند و جست‌وجو را تا پایانش متوقف می‌کند.

**نتیجه‌اش کجا می‌رود:** صف «کانال‌های پیشنهادی» با `source = 'telegram_search'`. هیچ
کانالی خودکار تأیید نمی‌شود؛ نتیجهٔ جست‌وجو یک سرنخ است نه یک تصمیم.

```sql
SELECT username, score, status FROM channel_candidates
 WHERE source = 'telegram_search' ORDER BY score DESC LIMIT 20;
```

**خاموش کردن:** همان فلگ روی `false`. کاندیداهای پیداشده سر جایشان می‌مانند.

## <a id="nginx-update"></a>بعد از `git pull`: کی باید nginx را از نو ساخت

**نشانه:** همه‌چیز سالم است (`ps` همه healthy، لاگ API پر از ۲۰۰)، ولی مینی‌اپ و پنل
«سرور جواب نداد» می‌دهند و `curl` روی `api.<دامنه>` کد **502** برمی‌گرداند.

**علت:** فایل‌های `infra/nginx/*.template` فقط **موقع استارت کانتینر** به کانفیگ
واقعی تبدیل می‌شوند. `up -d --build` ایمیج‌ها را می‌سازد و `api` را از نو می‌سازد، ولی
اگر تنظیمات nginx در compose عوض نشده باشد به کانتینر nginx دست نمی‌زند — پس nginx
با کانفیگ قدیمی می‌ماند.

```bash
docker compose -f infra/compose/solo.yml --env-file .env up -d --force-recreate nginx
```

**قاعده:** هر بار که pull تو فایلی زیر `infra/nginx/` را عوض کرد، همین یک دستور.
`nginx -s reload` کافی نیست وقتی خود *قالب* عوض شده باشد، چون reload فایل تولیدشدهٔ
قدیمی را دوباره می‌خواند، نه قالب را.

> کانفیگ فعلی نام کانتینرها را per-request از DNS داکر می‌خواند، پس بعد از یک بار
> force-recreate، ساخت دوبارهٔ `api` یا `edge` دیگر ۵۰۲ نمی‌دهد.

## <a id="more-accounts"></a>بیشتر کردن سرعت با اکانت دوم

هر سرویس edge **یک** سشن برای هر نقش برمی‌دارد (`crawl*` برای کرالر، بقیه برای
resolver) و سشن‌های اضافه را نادیده می‌گیرد — در لاگ `account.extra_sessions_ignored`
می‌بینی. پس گذاشتن دو فایل سشن کنار هم چیزی را سریع‌تر نمی‌کند؛ راهش یک edge دوم است.

```bash
C="docker compose -f infra/compose/solo.yml --env-file .env"
$C --profile edge2 run --rm edge2 python -m tmusic_indexer.login crawl2   # شمارهٔ دوم
$C --profile edge2 up -d edge2
```

`edge2` ولوم جدا دارد، پس سشن جدا دارد. lease در core جلوی این را می‌گیرد که دو
کرالر سراغ یک کانال بروند، و هر کدام با `worker_id` خودش کار می‌گیرد.

**چه چیزی سریع‌تر می‌شود:** کرال کانال‌های MTProto و جست‌وجوی تلگرام — دو برابر، چون
محدودیت تلگرام روی هر اکانت جداگانه است. کرال پیش‌نمایش وب هم موازی‌تر می‌شود، ولی آن
یکی به IP سرور محدود است نه به اکانت، پس اگر `crawl.blocked` دیدی گلوگاه IP است و
اکانت سوم کمکی نمی‌کند (پراکسی می‌کند).

**پخش را هم بین هر دو پخش کن:** به edge دوم یک سشن resolver بده (نامی که با `crawl`
شروع **نشود**):

```bash
$C --profile edge2 run --rm edge2 python -m tmusic_indexer.login acc2
$C --profile edge2 up -d edge2
```

هر دو سرویس روی نام مشترک `edges` alias دارند؛ DNS داکر برای این نام آدرس هر دو
کانتینر را برمی‌گرداند و بینشان می‌چرخد، پس هم nginx (استریم) و هم core (resolve و
probe و send) خودبه‌خود تقسیم می‌شوند — بدون فهرستی که لازم باشد نگه داری. اگر `edge2`
خاموش باشد، همان نام فقط به edge اول اشاره می‌کند و چیزی خراب نمی‌شود.

هر edge دو سشن دارد و هر کدام کار خودش را می‌کند: `crawl*` برای کرال، بقیه برای
resolve. یعنی روی دو ماشینِ مجازی چهار اکانت: دو تا کرال، دو تا پخش.

> اگر می‌خواهی اکانت کرالر و اکانت پخش کاملاً جدا بمانند (توصیهٔ اولیه)، روی edge دوم
> فقط `crawl2` را لاگین کن و سشن resolver ندهش. آن‌وقت پخش فقط روی اکانت اول می‌ماند.

**اکانت سوم و چهارم:** همین الگو، با `edge3` و `edge4` در فایل compose. اگر به آنجا
رسیدی، بهتر است به جای کپی کردن سرویس، استخر اکانت را برگردانیم — بگو تا بسازمش.
